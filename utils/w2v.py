...
if __name__ == '__main__':
    config = ConfigParser.get_config('../config/config.json')
    data_name = config['name']
    embed_path = config['data_loader']['args']["embed_path"]
    embedding_path = Path('..') / 'data' / data_name / embed_path
    print(embedding_path)
    emb = Embedding(embedding_path)
